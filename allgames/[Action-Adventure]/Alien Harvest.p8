	...
	if gameover<1 then
		if lvl.eggs>0 then
			...
			if egg_t==sec(1.25) then
				l_egg=get_rndtile(3)
				if l_egg then tile_attr(l_egg.tx,l_egg.ty,"s",70) end
			end
...
function intro_init()
	function intro_drw()
		fd_upd() 
		cprint("alien harvest v1.6;(c)brian vaughn, 2017;;design+code;brian vaughn;@morningtoast;;music;brian follick;@gnarcade_vgm;;animation;@pineconegraphic", 8, fd_c)
