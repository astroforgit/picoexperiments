...
function startgame()
 poke(0x3101,194)
 music"0"
 fadeperc,chain,chainsafe,buttbuff,logo_t,logo_y,hpwind,win,winfloor,turn,key,_upd,_drw,tmap,Œ,floats,wind,sprs,eqp,st_steps,st_kills=1,dget(0),true,0,240,35,nil,false,10,"ai",0,update_anis,draw_game,blankmap(1),addmob(1,20),{},{},{},{},0,0  
 genfloor(0)
end
-->8
--updates
...
function update_inv()
 ...
 elseif btnp"5" then
  if seleqp==nil then
   ...
   if tmap[Œ.pos+dirpos[thrdir]]<0 then
...
function drawarrow(dr)
 local x,y=postoscreen(Œ.pos+dirpos[dr])
 myspr(dr+170,
...
function hitmob(mob,dmg,stun)
 ...
 addfloat("-"..dmg,mob.pos,mob==Œ and 8 or 9)
 
 sfx(mob==Œ and 6 or 7)
 mob.vis=true
...
function unfog()
 for p=20,305 do
  if fog[p]==1 and dist(Œ.pos,p)<=Œ.sight and los(Œ.pos,p) then
   unfogtile(p)
  end
 end
end
...
function eat()
 local abl,pos,tdir=seleqp.typ,Œ.pos,dirpos[thrdir]
 ...
 elseif abl==4 then
  ...
  sp.trg_val,sp.trg_func,sp.trg_mob=land,mobhop,Œ
  sfx"11"
 elseif abl==5 then
  rope(pos,pos2)
  sfx"17"
...
function dohpwind()
 local hpy=Œ.pos<161 and 110 or 5
...
function ai_wait(m) 
 if sight(m,Œ.pos) then
  m.task,m.tpos,m.aicool,m.active,buttbuff=m.taska,Œ.pos,0,true,0
  addfloat("!",m.pos,10)
 end
 return false
end
...
function ai_dobump(m)
 local d=5
 if dist(m.pos,Œ.pos)==1 then
  repeat
   d-=1
  until dirpos[d]==Œ.pos-m.pos or d==1
  mobbump(m,d)
  hitmob(Œ,1)
  if m.typ==4 then
   blind(Œ)
...
function ai_attac(m)
 ...
 elseif ai_tcheck(m) then
  ...
  if cand>0 then
   mobwalk(m,cand)
   if m.typ==6 and dist(m.pos,Œ.pos)>1 and rnd(3)>1 then
    m.vis=false     
   end
   m.trig=true
   return true
...
function ai_reaper(m)
 if ai_dobump(m) then
  ...
  local cand=getnextstep(m.pos,Œ.pos)
  if cand>0 then
   mobwalk(m,cand)
   m.trig=true
   return true
...
function ai_queen(m)
 if not sight(m,Œ.pos) then
  ...
  if m.charge==0 then
   ...
   m.charge-=1
   local cand=getnextstep(m.pos,Œ.pos,true)
   if cand>0 then
    mobwalk(m,cand)
    m.trig=true
    return true
...
--gen
function genfloor(f)
 buttbuff,steps,floor,turn,mobs,dmobs,picks,floats,wind,fog,voidmap=0,0,f,"player",{Œ},{},{},{},{hpwind},blankmap(0),blankmap(0) poke(0x3101,66)
 if floor==0 then  
  copymap(0,0)
  addmob(15,153)
...
function mapgen()
 ...
 tmap[spawnpos],Œ.pos,tmap[epos]=15,spawnpos,14
