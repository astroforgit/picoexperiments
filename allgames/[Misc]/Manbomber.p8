...
function _selectionScene:update()
    if btnp(”) and self.cursorPos > 0 then 
		sfx(04)
		self.cursorPos -= 1
	end
	if btnp(ƒ) and self.cursorPos < 2 then 
		sfx(04)
		self.cursorPos += 1
    end
    if btnp(—) then 
...
function _winSelectionScene:update()
    ...
    if not self.selected then
        if btnp(”) and self.winNumber < 9 then
            sfx(4)
            self.upArrowY += 1
            self.winNumber += 1
        end
        if btnp(ƒ) and self.winNumber > 1 then
            sfx(4)
            self.downArrowY -=1
            self.winNumber -= 1
        end
        ...
        if btnp(—) then
            sfx(1)
            mainScene.neededWins = self.winNumber
