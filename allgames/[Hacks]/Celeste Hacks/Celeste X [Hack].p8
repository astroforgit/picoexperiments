...
player = 
{
  ...
  update=function(this)
    ...
    local dash = btn(k_dash, 1) and not this.p_dash
    this.p_dash = btn(k_dash, 1)
...
maddy = {
  ...
  update=function(this)
    ...
    elseif this.state==5 then
      ...
      local v_input=0
      this.p=get_player()
      if abs(this.spd.y<0.15) and mrng(1.0)<0.05 then
        dash=true
      ...
      if this.timer>0 then
        ...
        if p~=nil then
          if abs(p.x-this.x) <= 60 and abs(p.y-this.y) <= 4 then
            this.dir=p.x>this.x and 1 or (p.x<this.x and -1 or this.dir)
            if this.is_solid(0,1) and not this.is_solid(0,-8) and rnd(1)<0.33 then
              this.spd.y=-1.5
            end
            this.state=1
            this.timer=30
