...
function is_title()
	return level_index()==31 and start_game_flash>-30 and start_game_flash<100
end
...
player = 
{
	...
	update=function(this)
		...
		-- next level
		if (this.y<-7) and level_index()!=31 then kill_player(this) end
		...
		if on_ground and not this.was_on_ground then
		 init_object(smoke,this.x,this.y+4,0)
		end
  ...
  if this.dash_time > 0 then
   init_object(smoke,this.x,this.y,0)
  	this.dash_time-=1
  	...
  	if abs(this.spd.y) <= 0.15 then
   	gravity*=0.5
			...
			if input!=0 and this.is_solid(input,0) and not this.is_ice(input,0) then
		 	maxfall=0.4
		 	if rnd(10)<2 then
		 		init_object(smoke,this.x+input*6,this.y,0)
				end
			end
		  	...
		  	this.grace=0
					...
					if wall_dir!=0 then
			 		...
			 		if not this.is_ice(wall_dir*3,0) then
		 				init_object(smoke,this.x+wall_dir*6,this.y,0)
						end
					end
				end
			end
...
player_spawn = {
	...
	update=function(this)
				...
				this.spr=6
				destroy_object(this)
				init_object(player,this.x,this.y,this.spd.y)
	end,
	draw=function(this)
...
function break_fall_floor(obj)
 if obj.state==0 then
 	psfx(15)
		...
		init_object(smoke,obj.x,obj.y,0)
	 ...
	 local hit=this.collide(player,0,0)
		if hit~=nil then
		 hit.djump=max_djump
			...
			init_object(lifeup,this.x,this.y,0)
			destroy_object(this)
		end
		this.off+=1
...
fake_wall = {
	...
	update=function(this)
		...
		if hit~=nil and hit.dash_effect_time>0 then
			...
			destroy_object(this)
			init_object(smoke,this.x,this.y,0)
			init_object(smoke,this.x+8,this.y,0)
			init_object(smoke,this.x,this.y+8,0)
			init_object(smoke,this.x+8,this.y+8,0)
			init_object(fruit,this.x+4,this.y+4,0)
		end
	...
	update=function(this)
		if has_key then
			...
			if this.timer<=0 then
			 sfx_timer=20
			 sfx(16)
				init_object(fruit,this.x,this.y-4,0)
				destroy_object(this)
			end
		end
	end
...
message={
	...
	last=0,
	draw=function(this)
		this.text="-- celeste mountain --#this memorial to those# perished on the fall "
...
flag = {
	...
	draw=function(this)
		...
		spr(this.spr,this.x,this.y)
		if this.show then
			rectfill(2,97,66,126,0)//32-96 2-31
			spr(26,25,101)
			gotflag=true
			print("x"..this.score,34,104,7)
			draw_time(19,111)
			print("deaths:"..deaths,18,119,7)
	...
	draw=function(this)
		...
		elseif this.delay<0 then
			...
			elseif level_index()==30 then
				...
				local level=(1+level_index())%32*100
...
function init_object(type,x,y,speed)
...
function next_room()
	...
	last=0,
	draw=function(this)
		this.text="---  0m  memorial  ---#this memorial made by:#        kamera        "
		if this.check(player,4,0) then
			if this.index<#this.text then
			 this.index+=0.5
				if this.index>=this.last+1 then
...
function load_room(x,y)
	...
	for tx=0,15 do
		for ty=0,15 do
			...
			if tile==11 then
				init_object(platform,tx*8,ty*8,0).dir=-1
			elseif tile==12 then
				init_object(platform,tx*8,ty*8,0).dir=1
			else
				foreach(types, 
				function(type) 
					if type.tile == tile then
						init_object(type,tx*8,ty*8,0) 
					end 
				end)
			end
		end
	end
			init_object(room_title,0,0,0)
end
...
function _update()
	...
	frames=((frames+1)%30)
	if frames==0 and gotflag==false and start_game_flash<100 then
...
function _draw()
	...
	rectfill(128,-5,133,133,0)
		...
		-- credits
	if is_title() and start_game_flash>-30 then
		cls()
		spr(73,36,30,7,4)
		print("x+c",58,64,5)
		print("original game by",32,80,5)
		print("maddy thorson",38,87,5)
		print("noel berry",44,94,5)
		print("mod created by",36,110,5)
		print("gamergirlevalynn",32,118,5)
	end
...

...

...

...
0000b302b200000000b10000000000a200000000000000009300001000000000846282828283828282132323528452526292000000112434440000a282425284
...
000000b10000001100100000000000000000000093000086820000a3000000005262828201a200a282829200132323236211111111243535450000b312525252
...

...

...
000100110092a2829211a2133300a3825262b2000000a21333b20000868242520000000000000100009300000000000083828282000000a24322222232428452
...

...

...
001100000000b1a2828273b200000000232333132333b200001111000000b342000000868382125252328293a300000000670000a20192647400a20242525252
...
b302b211000000110092b100000000a3b1b1b1b1b1b10011111232110000b342000000a2821252845252328283860000f31232d30000e36575e3000013525252
...
00b1b302b200b372b200000000000082b20000000000b31222522363b200b3138585868292425252525262018282860022846202000672020272f300a2428452
...
000000b10000b303b200000000858682b27100000000b3425233b1b1000000b182018283001323525284629200a2820052525232161252222252320016135252
...

...
628201729300000000a282828382828252528462b20000a38300a382018283820000425252525252525284525222223252525252525252624252525252525252
__gff__

__map__

...

...

...

...

...

...

__sfx__

