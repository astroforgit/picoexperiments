...
function _init()
  ...
  buttons={o=emulated and "z" or "é",x=emulated and "x" or "ó"}
end
...
function _update()
  ...
  elseif mode==modes.main_menu then
    if (btnp(î)) menu_selection=menu_selection==1 and 3 or menu_selection-1
    if (btnp(É)) menu_selection=menu_selection%3+1
    if btnp(é) then
  ...
  elseif mode==modes.custom_submenu then
    if (btnp(î)) menu_selection=menu_selection==1 and 2 or 1
    if (btnp(É)) menu_selection=menu_selection%2+1
    if btnp(é) then
  ...
  elseif mode==modes.tutorial then
    if (btnp(é) or btnp(ó)) mode=modes.main_menu
  elseif mode==modes.level_select then
    ...
    if (btnp(î) and lvl>#level_select[1]) lvl-=#level_select[1] lvl_ypos-=8
    if (btnp(É) and lvl<=level_select.lvls-#level_select[1]) lvl+=#level_select[1] lvl_ypos+=8
    if btnp(é) then
      if edit then
        if levels[lvl] then
  ...
  elseif mode==modes.fail_state then
    counter+=1
    if (btnp(é)) mode=modes.play turn_off_draw()
  elseif mode==modes.resize then
    menuitem(1,"level select",function() turn_off_draw() pal() mode=modes.level_select end)
    menuitem(2)
    local x,y=#empty_level[1],#empty_level
    if (btnp(î) and y>2) empty_level[y]=nil
    if (btnp(É) and y<8) then
      local new_row={0}
      for i=2,x do
        add(new_row,1)
      end
    ...
    if (btnp(ë) and x<8) foreach(empty_level,function(x) add(x,1) end)
    if (btnp(ã) and x>2) foreach(empty_level,function(x) x[#x]=nil end)
    if (btnp(é)) mode=modes.edit_custom play_init(empty_level)
    if (btnp(ó)) mode=modes.level_select
  ...
  elseif mode==modes.edit_password then
    ...
    elseif btnp(É) and current_char!="ok" then
    ...
    elseif btnp(é) and (char<=10 and line <=3) then
      if current_char=="left" then
