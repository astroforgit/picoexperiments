...
function guy:bound()
 ...
 if self.supported_by then
  if(btnp(‹) and self.state~="left") self:become("left") g_moves+=1 sfx(20)
  if(btnp(‘) and self.state~="right") self:become("right") g_moves+=1 sfx(20)
  if(btnp(ƒ) and self.state~="idle") self:become("idle") g_moves+=1 sfx(20)
  if(btnp(”) and self.state~="up") self:become("up") g_moves+=1 sfx(20)
 end
...
function ingame.draw()
 ...
 if(btn(Ž))oprint(stat(1),2,2,7,6)
end
