...
function _draw()
 ...
 if (btn(Ž) and debug_wob_encode) then
...
function process_btns()
 ...
 if (msy > menu_y+52 and btn(Ž)) 
 then
	
	 if (btnp(‹)) undo() show_msg(30,"undo")
	 if (btnp(‘)) redo() show_msg(30,"redo")
	 if (btnp(”)) resize_brush(1) show_msg(30,"size:"..brush_size())
  if (btnp(ƒ)) resize_brush(-1) show_msg(30,"size:"..brush_size())
...
function update_system()
 ...
 if (not btn(Ž) or msy<menu_y+52) then
 ...
 if (msy>menu_y+52) then
  if (btn(Ž)) block_x=true
 end
 
 if (not btn(—) and not btn(Ž))
