...
function is_core_online()
	return mget(map_size/2,map_size/2)==21
end
...
function find_active_purifiers(x,y)
	local core_area = {}
	for mapx=map_size/2-3,map_size/2+3 do
		for mapy=map_size/2-3,map_size/2+3 do
			if is_purifier(mget(mapx, mapy)) and is_active(mapx, mapy) then
				add(core_area, {x=mapx, y=mapy})
			end
		end
	end
	return core_area
	...
	elseif gamestate == 1 then
		...
		if mode==0 then
			...
			if btn(5) then
				x_released=false
				x_down = x_pressed==0
				if x_pressed==0 then x_down_at = {cursor_x, cursor_y} end
				x_pressed=x_pressed + 1
			else
				x_released = (x_pressed>0 and x_down_at[1]==cursor_x and x_down_at[2]==cursor_y)
				x_pressed=0
			end
			
			if is_transport(mget(cursor_x, cursor_y)) then
				if x_down then
					button_down_at_transport = {cursor_x, cursor_y}
				end
			end
				...
				then
					...
					elseif x_pressed>0 then
						if selected_building[tile]~=0 then
							...
							msg_str = "press o to select building"
							msg_res=60
						end
					end
			...
			if is_transport(mget(cursor_x, cursor_y)) then
				if x_released
						and button_down_at_transport ~= nil
						and button_down_at_transport[1]==cursor_x
						and button_down_at_transport[2]==cursor_y then
					sfx(12)
...
function get_selected_building()
	local tile = mget(cursor_x,cursor_y)
	local land = get_land_of_tile(tile)
	return selected_building[land]
end
...
function draw_map()
	...
	if draw_load>0.8 and cpu_load>1 then
	...
	for x=reduce_load,16-reduce_load do
		for y=reduce_load,14-reduce_load do
			...
			then
				...
				if is_core_online() then
					local corex=map_size/2
					local corey=map_size/2
					if mapx>13 and mapx<33 and mapy>13 and mapy<34 then
						local testx=corex+(mapx-corex)*5
						local testy=corey+(mapy-corey)*5
						if 16==mget(testx+map_size, testy) then
							pal(0,2) palt(0,false)
						end
					end
				end
...
function draw_map_mini(highlights)
	...
	pset(13+cursor_x,33+cursor_y,8)
