...
function cursor_input()
 ...
 if btnp(ã) then
  ...
  crs.y = mid(0, crs.y +  1,  grid.size - 1)
	...
	if btn(ó) == false and crs.drawing then
	 end_drawing()
	end
...
function _init_tile_types()
 ...
 add_type_belt("ëë", "ãã", 65, 80)
 add_type_belt("ÉÉ", "îî", 64, 81)
 
 add_type_belt("ëÉ", "îã", 97, 99)
 add_type_belt("Éã", "ëî", 113, 115)
 add_type_belt("ãî", "Éë", 112, 114)
 add_type_belt("îë", "ãÉ", 96, 98)
...
function menu_input()
 if menu.grace == 0 then
		if btnp(É) then
		 menu.sel += menu.sel_îÉ
		end
		if btnp(î) then
		 menu.sel -= menu.sel_îÉ
		end 
		if btnp(ë) then
		 menu.sel += menu.sel_ãë
		end
		if btnp(ã) then
		 menu.sel -= menu.sel_ãë
		end
...
function draw_recipes_menu()
 ...
 print("èrecipesè", left + 15, top + 5, 1)
...
function draw_elevator_menu()
 ...
 if cur_goal > #goals then
  print("è you win! è", left + 28, top - 8, 1)
 else
  print("è goal "..cur_goal.." of "..#goals.." è", left + 21, top - 8, 1)
 end
...
function draw_inv_menu()
 ...
 print("èinventoryè", left + 3, top + 4, 1)
...
function draw_tut_1(left, top)
 ...
 print("è tutorial è", left + 28, top - 8, 1)
...
function draw_tut_2(left, top)
 draw_tut_window(left, top)
 print("è great work! è", left + 21, top - 8, 1)
 top += 6
 left += 4
...
function draw_tut_3(left, top)
 draw_tut_window(left, top)
 print("è powerful! è", left + 26, top - 8, 1)
 top += 6
 left += 4
...
function draw_tut_4(left, top)
 draw_tut_window(left, top)
 print("è heck yea! è", left + 26, top - 8, 1)
 top += 6
 left += 4
...
function draw_tut_5(left, top)
 ...
 print("è nice! è", left + 30, top - 8, 1)
 top += 6
 left += 4
...
function draw_main()
 ...
 sspr(0, 96, 127, 31, 0, 16)
 print(tick("è start", "   start", 1), 40, 61, 9)
