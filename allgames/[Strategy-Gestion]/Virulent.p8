...
local function curtains(d,t,l,bt)
 if not t then
  print("óSKIP",1,122,13)
  if d.f then
   print("óSKIP",1,122,12)
  end
...
function command_screen.update()
 ...
 elseif btnp(ó) then
  goto_region()
 elseif actno==0 or (done and btnp(é)) then
  goto_report()
 end
end
-->8
...
function region_screen.init(no_anim)
 ...
 if needs_remedy() and actno>0 then
  add(toolbar,{244,"é APPLY REMEDY ",goto_remedy})
 end
 printw(29)
 print_title(cname,9)
 lock_tb()
...
function region_screen.update()
 update_tb()
 if btnp(é) and needs_remedy() then
  goto_remedy()
 end
end
-->8
...
function remedy_screen.update()
 update_tb()
 if btnp(î) or btnp(É) then
  _i=rot_sel(_i,_o)
  set_redraw()
 elseif btnp(ã) or btnp(ë) then
  set_redraw()
 elseif btnp(é) then
  apply_remedy()
 end
...
function help_screen.draw()
 if can_redraw() then
  ...
  par"SELECT A COUNTRY AND PRESS ó"
  ...
  print("îÉ SCROLL",1,121,1)
  print("ó CLOSE",96,121,1)
  -- progress
