...
function guy:move()
 ...
 if (btnp(ƒ) and self.supported_by and self.vert) self:flip()
 if (btnp(”) and self.supported_by and not self.vert) self:flip()
...
function ingame.draw()
 ...
 if g_index==0 then
  coprint("‹‘ ^move  ",25,(btn(‹) or btn(‘)) and 6 or 5)
  coprint("— ^toggle gravity ",96,btn(—) and 6 or 5)
  coprint("”ƒ ^change gravity  ",106,(btn(ƒ) or btn(”)) and 6 or 5)
 elseif g_won and g_index==30 then
  ...
  coprint("Œ "..g_deaths.." deaths ",y+50,8)
 ...
 if(btn(Ž))oprint(stat(1),2,12,stat(1)>=1 and 8 or 11)
end
...
function menu.draw()
 ...
 if(btn(Ž)) oprint(stat(1),2,2,stat(1)>=1 and 8 or 11)
end
