...
function draw_menu()
 ...
 print_centre("press Ž for help",105,15)
 print("Š",31,112,12)
 ...
 if delay<=100 and	btnp(—) then
   _initall()
	...
	if (score_entry.entering) then
		...
		if (btnp(‘)) score_entry.entry_character = min(3, score_entry.entry_character+1)
		if (btnp(‹)) score_entry.entry_character = max(1, score_entry.entry_character-1)
