...
-- music and sound by brian follick, @gnarcade_vgm
...
background={t=0,
	list={"grid","balls","flurry","ballspiral","flight","circles","rainbow","spiral"},
...
-- not a background, do not alter below
	...
	load=function()	
		...
		if gridcolor==12 then
			bgp=rand_table(background.list)
		end
...
--for snow
rand,rand2={},{}
for n=1,256 do add(rand, rnd(128)) end
for n=1,256 do add(rand2, rnd(128)) end

showsnow,snowx,snowy=true,0,0
function snow()
	if showsnow then
		snowx+=.5
		snowy+=.5
		for i=1,18 do
			a=rand[i]+snowx+sin(i/32+time())
			b=rand2[i]+snowy
...
-- globals
--
version="1.7.2" --updated 7/5/2017
t=0
...
player={
	...
	_update=function()
		...
		if player.weapon>0 then
			player.weapontime+=1
			if player.weapontime>=player.weaponmax then 
		...
		if p_shields then
			shield_xmin=player.pos.x-4
			shield_xmax=player.pos.x+10
			shield_y=player.pos.y-5
		end
	...
	_draw=function()
		...
		if player.st==1 then
			...
			if p_shields then
				rectfill(player.pos.x-4,player.pos.y-5, player.pos.x+10,player.pos.y-4, 7)
			end
	...
	changeweapon=function(w)
		...
		--base weapon
		if player.weapon==0 then --4 is shields, use regular gun
			player.zrate=8
		end
	...
	action=function()
		...
		if btnz and player.st<3 then -- z button
			...
			if (player.weapon!=1) and player.t.z>=player.zrate then
				sfx(61)
...
bullets={
	...
	_draw=function()
		...
		foreach(bullets.all, function(obj)
			-- normal shot
			if obj.weapon==0 or obj.weapon==4 then
	...
	create=function(x,y)
		...
		-- normal 
		if player.weapon==0 or player.weapon==4 then --4 allows normal fire while shields
...
-- #ufo ship
--
ufos={t=0,spawn=90,active=false,current={pos={x=0,y=0}},score=5000,
	_update=function()
		if ufos.active then
	...
	die=function()
		...
		if not player.fever then
			powerup.drop(ufos.current.pos.x,ufos.current.pos.y, 1,99) --always drop yellow block
		end
	end,
	kill=function()
		ufos.active=false
...
mobs={all={},t=0,dir=1,move=20,
	...
	die=function(mob)
		...
		if player.fever then --when in fever mode, always drop coins
			powerup.drop(mob.pos.x,mob.pos.y, 5,99)
		end
	end,
	kill=function(mob)
...
shots={
	...
	_update=function()
		foreach(shots.all, function(obj)
			...
			if p_shields then
				local shoty=obj.pos.y+5
					
				if obj.boss then
					shoty=obj.pos.y+23	
				end
					
				if obj.pos.x>=shield_xmin and obj.pos.x<=shield_xmax and shoty>=shield_y then
					shots.remove(obj)
				end
			end
		end)
...
powerup={
	...
	drop=function(x,y,qty,putype)
		...
		if not putype then putype=flr(rnd(4))+1 end --get power-up color
		...
		for n=1,qty do
			...
			if putype==4 then
				...
				pu.apply=function()
					...
					if not p_shields then p_shields=true end
					player.weaponcolor=pu.color[1]
				end
			end
...
menuitem(1, "toggle music", function() 
	if musicon then musicon=false music(-1) else 
		musicon=true 
...
intro_draw=function()
	center_text("invader overload "..version,8,textc)
	center_text("(c)2016-17 brian vaughn",16,textc)
	...
	center_text("a bullet factory game",110,textc)
	
	if t>6 then textc=5 end
	if t>10 then textc=6 end
	if t>16 then textc=7 end
	
	if t>100 then textc=6 end
	if t>104 then textc=5 end
	if t>110 then textc=0 end
	
	if t>120 then titlescreen._init() end
...
titlescreen={
	...
	_update=function()
		...
		if btnp(2) then
			--if showsnow then showsnow=false else showsnow=true end
			if gridcolor==8 then 
				gridcolor=12
			else
				gridcolor=8
		...
		if btnxp then
			if musicon then music(-1) musicon=false else musicon=true music(32) end
		end
	end,
...
gameover={
	...
	_draw=function()
		...
		center_text("your score",35,10)
		center_text(score_text(player.score),43,10)
		
		center_text("high score",58,7)
		center_text(score_text(highscore),64,7)
		
		--center_text("happy holidays!",58,7)
		--center_text("be safe and have fun",65,7)
		
		
		if gameover.t>=gameover.wait then
			center_text("press "..char.btnz.." or z to continue",90,0)
		end
		
		center_text("animated backgrounds by",110,7)
		center_text("the pico-8 community",118,7)
	end
}
...
function _init()
	...
	if firstplay then intro_init() firstplay=false else titlescreen._init() end
...
__gfx__

