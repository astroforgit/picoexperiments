...
cuelen,cuerad1,cuerad2,
cuemin,cuemax,cuedef,cuestep=100,1.25,2,1,25,10,0.25
mode_aim,mode_sim,mode_postgame,mode_menu,mode_placeball=1,2,3,4,5
etable,eball,friction=0.7,0.55,0.05
allverts={}

-- variables
fov,camang,cueang,camzoom,campitch,
cuepow,centercamf=128,0,0,100,-0.135,cuedef,0
...
function newgame(npc,npc2)
 ...
 players={
  {
  	name=npc2 and npc2.name or "player1",
  	ballct=0,
  	npc=npc2
  },
  {
   name=npc and npc.name or "player2",
   ballct=0,
   npc=npc
  }
 }
 ...
 if(npc and rnd(2)>=1)swapplayers()
 ...
 while step>0 do
  ...
  for i=1,#balls do
   ...
   if isballmoving(ball) then
				...
				-- edge collisions
			 if ball.bounds.mn[1]<=-zonew
			 or ball.bounds.mn[2]<=-zonel
			 or ball.bounds.mx[1]>= zonew
			 or ball.bounds.mx[2]>= zonel then
...
-- 			local iv2= coln*i/2
 			...
 			if not firsthit then
 			 if(colball==white)firsthit=colball2
 			 if(colball2==white)firsthit=colball
 			end
...
function endshot()
 ...
 elseif not firsthit then
  ...
  if reqtyp=="overs" and prevoverct==0
  or reqtyp=="unders" and prevunderct==0 then
   reqtyp="black"
  end    
 ...
 if not foul then
	 if player.balltyp=="overs"  and sunkunder
	 or player.balltyp=="unders" and sunkover then
 ...
 if(player.balltyp=="overs"  and sunkover) continue=true
 if(player.balltyp=="unders" and sunkunder)continue=true
 
 -- cannot continue after a foul
 if foul then
  continue=false
  freeball=true		-- other player gets a free ball
 end
...
function isanyballmoving()
 for ball in all(balls) do
  if(isballmoving(ball))return true
 end
 return false
end
...
function ballcorner(ball,corner)
 ...
 if((l*ball.vel)>=0)return		-- ball moving away
...
function getinput()
 ...
 if (xinp<0 and xi>=0) or (xinp>0 and xi<=0) then
  ...
  xinp+=xi*0.04
	...
	xinp=clamp(xinp,-1,1)
	yinp=yinp*.9+yi*.1
	if(abs(xinp)<0.001)xinp=0
	if(abs(yinp)<0.001)yinp=0
	return xinp,yinp
end
...
function gamedone()
 ...
 if players[1].npc then
  ...
  -- otherwise switch to post game mode
	 initmenu() 
	 if players[1].npc then		-- demo mode?
	 	mode=mode_menu
	 else 
		 mode=mode_postgame
		end 
...
function updatepostgame()
 ...
 if(option==1)newgame(players[2].npc,players[1].npc)
 if(option==2)mainmenu()
end
...
function updatemenu()
 ...
 menutimer+=1 
 if(menutimer>3000)demo()
 
 -- otherwise
 local option=domenu(3)
 if(option==1)newgame(rnd(npctypes))
 if(option==2)newgame()
 if(option==3)demo()
end
...
function domenu(ct)
 if btnp(”) and menuoption>1 then
 	menuoption-=1
 	menutimer=0
 	sfx(11)
 end
 if btnp(ƒ) and menuoption<ct then
  menuoption+=1
 	menutimer=0
  sfx(11)
 end
...
function smoothmovecam(targetcam,targetzoom,targetfov,targetang)
 ...
 if(targetcam)cam=cam*0.8+targetcam*0.2
	if(targetfov)fov=fov*0.8+targetfov*0.2
 if(targetzoom)camzoom=camzoom*0.95+targetzoom*0.05
end
...
function rendertable(drawcue)
 ...
 cammat=translate(0,yoffs,camzoom)
					 	*rotx(campitch)
							*rotz(-camang)
							*translate(-cam)
	
	-- camera pt in world space
	local camw=cammat:cheapinverse()
							    :transformpt(vec(0,0,0))
 ...
 for m in all(cushionmeshes) do
  rendermesh(m)
	...
	if(camw[2]>-tablel/2)rendermesh(cushiontopmesht)
	if(camw[2]< tablel/2)rendermesh(cushiontopmeshb)
	if(camw[1]>-tablew/2)rendermesh(cushiontopmeshl)
	if(camw[1]< tablew/2)rendermesh(cushiontopmeshr)
 ...
 -- cue
 if(mode==mode_aim)cuepos=white.pos
 
 if drawcue then
	 local d=ballrad
	 if(mode==mode_aim)d+=cuepow
...
function drawplayerui(p,left)
 ...
 if left then
	...
	if p.balltyp then
	 ...
	 if(mode~=mode_sim)p.ballct=min(p.ballct+1/8,ct)
...
function drawinputhints()
 if (mode==mode_aim or mode==mode_placeball) 
 and inputhints 
 and not player.npc then
  hinty+=2
 else 
  hinty-=2
 end
 ...
 if mode==mode_aim or mode==mode_sim then
	 if mode==mode_aim and btn(Ž) then
	  print("”",4,hinty-3,6)
	  print("ƒ",4,hinty+3,6)
	...
	elseif mode==mode_placeball then
	 print("”",10,hinty-3,6)
	 print("ƒ",10,hinty+7,6)
	 print("‹ ‘",4,hinty+2,6)
	 prettyprint("move ball",26,hinty+2,7)
  print("—/Ž",86,hinty+2,6)
...
function renderpoly(poly)
 ...
 for v in all(poly.verts) do
  ...
  if pv.proj and not v.proj 
  or not pv.proj and v.proj then
 ...
 while y<64 do
  ...
  local lrx=ceil(lx)
  local rrx=flr(rx)
  if(rrx>=lrx)rectfill(lrx,y,rrx,y)
  lx+=lxd
  rx+=rxd
  y+=1
 end  
...
function makebounds(pt,size)
 size=size or 0
 return {
 	mn={pt[1]-size,pt[2]-size},
 	mx={pt[1]+size,pt[2]+size}
 }
end
...
function ptinbounds(bounds,pt)
 return bounds.mn[1]<=pt[1] and bounds.mn[2]<=pt[2]
    and bounds.mx[1]>=pt[1] and bounds.mx[2]>=pt[2]
end

function boundsintersect(b1,b2)
 return b1.mx[1]>b2.mn[1] and b1.mn[1]<b2.mx[1]
    and b1.mx[2]>b2.mn[2] and b1.mn[2]<b2.mx[2]
end
...
function initmenuitems()
 menuitem(
  2,
  "show keys:"..(inputhints and "on"or"off"),
  function()
 ...
 menuitem(
 	3,
 	"balls:"..(
 		   aligncirc==0 and "sub-pixel" 
 		or aligncirc==1 and "snap" 
 		or                  "snap-half"),
 	function()
...
function makebasemesh(tw,tl)
 ...
 return makemesh(
  bvecs,
  {
			{col=3,i=split"1,2,3,4"},
   {col=0,i=split"8,7,6,5"},
   {col=5,i=split"2,1,5,6"},
   {col=6,i=split"3,2,6,7"},
   {col=5,i=split"4,3,7,8"},
   {col=6,i=split"1,4,8,5"}
  })
end
...
function makemarkingsmesh() 
 ...
 -- baulk line vertices
 local bvecs={
 	vert(-tw,baulk,ballrad),
 	vert( tw,baulk,ballrad),
 	vert( tw,baulk-2,ballrad),
 	vert(-tw,baulk-2,ballrad)
 }
 local polys={
  {col=5,i=split"1,2,3,4"}
	}
...
function makepocketmesh(pocket,ch)
 ...
 for i=pocket.from,pocket.to do
  local v=table[i].pt
  add(verts,vert(v+vec(0,0,ballrad-ch)))  -- rim vertex
  add(verts,vert(v+vec(0,0,ballrad)))     -- inside vertex
 end
...
function makecushionmesh(pocket1,pocket2,ch)
 ...
 local verts={
 	vert(v1+vec(0,0,ballrad-ch)),
 	vert(v1+vec(0,0,ballrad)),
 	vert(v2+vec(0,0,ballrad-ch)),
 	vert(v2+vec(0,0,ballrad))
 }
 return makemesh(
 	verts,
 	{
 		{col=0x3b,pat=0xa5a5,i={1,2,4,3}}
 	}) 	
end
...
function makecushiontopmesh(verts,i1,i2)
 ...
 if from<to then
	 ...
	 polys=concat(
	 	tesellatestrip(from,#verts,4,nil),
	 	tesellatestrip(1,to,4,nil))
...
function makecuemesh()
 ...
 for i=0,1,cuestep do
  ...
  add(cueverts,{
  	pt=vec(s*cuerad1,0,c*cuerad1)
  })
  add(cueverts,{
   pt=vec(s*cuerad2,cuelen,c*cuerad2)
  })
 end
...
function makeballs()
 ...
 for r=0,4 do
  for c=0,r do
   ...
   -- create ball
   local ballpos=pos
   	+vec(-r/2+c,-r)*rackspacing
    +vec(rnd(2)-1,rnd(2)-1)*rackjitter
   local ball={
    pos=ballpos,
	   vel=vec()
	  }
...
function addsidepocket(x)
 ...
 for a=0.25,0.75,pocketstep do
  local r=0--max(abs(a-0.5)-0.22,0)
  local sharp=r>0
  r=(r*r*100+1)*pocketrad
...
function rayline(r,d,l1,l2)
 ...
 if(sgn(nr)==sgn(nd))return nil								-- ray facing away
 if(sgn(nr)==sgn(nr+nd*100))return nil -- ray doesn't reach line after 100 units
...
-- end,
 __index={
  transformvec=function(s,v)
   return s[1]*v[1]+s[2]*v[2]+s[3]*v[3]
  end,
  transformpt=function(s,v)
   return s:transformvec(v)+s[4]
  end,
  transposed=function(s)
...
function aiaim()
 ...
 local reqtyp=player.balltyp
 if reqtyp=="overs" and overct==0
 or reqtyp=="unders" and underct==0 then
  reqtyp="black"
 end
 ...
 for i,ball in ipairs(balls) do
  ...
  if(ball==white)goto aiskipball
  if(reqtyp and reqtyp~=ball.typ)goto aiskipball
  if(not reqtyp and ball==black)goto aiskipball
...
--   end
   ...
   if(abs(x[1])<=tabw and abs(x[2])<=tabl)wEIGHT+=8
   if(not aiisblocked(a,x,ball))wEIGHT+=4
   if(not aiisblocked(b,p,ball))wEIGHT+=2
   if(sA<=cuemax)wEIGHT+=1
...
::aiskippocket::
   if(best>=15 or isbreak)aimovecue(bestang)
   yield()
  end
...
function aiisblocked(p,t,ignoreball)
 p/=16			-- avoid numeric overflow
 t/=16
 local v=t-p
 
 for b in all(balls) do
  if b~=white and b~=ignoreball then
   local r=p-b.pos/16		-- position rel to ball
