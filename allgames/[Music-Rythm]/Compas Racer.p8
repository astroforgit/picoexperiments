...
function _init()
 ...
 texto[20]={"se comienza a usar el boton Ž","button Ž is now in use"}
 texto[21]={"ultima ronda","last round"}
 texto[22]={"ronda","round"}
 texto[23]={"ahora utilizamos Ž y —","now we use both Ž and —"}
 texto[24]={" de 3"," of 3"} 
		  ...
		  if n!=jugador then
		   ...
		   if rnd(4)<=1 then
		    --avanza segun la dificultad
			   corredor[n][1]-=rnd(1.4+dificultad/2)
  ...
  if primero!=-1 then
   if primero==jugador then color=8 else color=7 end
    ...
    if segundo==jugador then color=8 else color=7 end
	   ...
	   print(adecimales(time()-contadortiempo).." "..texto[33][idioma]..", "..corredor[jugador][6].." pts",8,110,7)
	 end
	end
end
